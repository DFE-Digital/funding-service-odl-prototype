...
logger.add(
    "logs/gias_pipeline_{time:YYYY-MM-DD}.log", rotation="10 MB", level="DEBUG"
)


class GIASIngestion:
    """Class to ingest GIAS data"""

    def __init__(
        self, base_url: str, datasets: dict, storage_dir: str = "GIAS_data"
    ):
    ...
    def clean_data(self, data: bytes) -> DataFrame:
        ...
        try:
            df = pd.read_csv(
                io.BytesIO(data), encoding="latin1", low_memory=False
            )
    ...
    def save_to_json(self, df: DataFrame, dataset_name: str) -> None:
        ...
        try:
            json_file_path = (
                self.storage_dir / f"{dataset_name}_{self.today}.json"
            )
            df.to_json(
                json_file_path, orient="records", date_format="iso", indent=4
            )
            logger.success(
                f"Json file written for {dataset_name}"
                f" written to {json_file_path}"
            )
        except Exception as e:
    ...
    def run(self):
        ...
        for dataset, table_name in self.datasets.items():
            try:
                ...
                logger.error(
                    f"Pipeline component '{dataset}' failed processing."
                )
        ...
        if self.failed_datasets:
            logger.warning(
                f"Ingestion completed with {len(self.failed_datasets)}"
                f"dataset failure(s)."
            )
        else:
            logger.success(
                "Injestion completed successfully for all datasets."
            )
...
def run_ingestion(
    ...
    gias_datasets: Dict[str, str] = {
        ...
        "grouplinks_edubaseallacademiesandfree": (
            "academy_sponsor_and_trust_links"
        ),
    ...
    gias_injestion = GIASIngestion(
        base_url=gias_base_url, datasets=gias_datasets
    )
